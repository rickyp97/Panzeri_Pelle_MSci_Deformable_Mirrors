...
void AddMenu(HWND hwin) {
    ...
    AppendMenu( hmenu, MF_STRING | MF_ENABLED, ID_13MHZ  , "13 Mhz" );
    ...
    AppendMenu( hmenu, MF_STRING | MF_ENABLED, ID_06MHZ  , " 6 Mhz" );
}
...
*/ 
        ...
        case WM_SYSCOMMAND:
        {
            ...
            switch (wParam) { 
                ...
                case ID_32MHZ: CxSetFrequency(SMX,10); return 0;
        ...
        case WM_TIMER: 
            switch (wParam) { 
                ...
                case IDT_TIMERFPS: 
                {
                    ...
                    CxGetFrameRate(SMX,&fps);
                    sprintf(buf,"%d fps [%7.3f]", (frameCount - savedCount) / TIMER_INTERVAL, fps );
...
static void ThreadProc(void *foo) {
    static TRgbPix frameOut[MaxFrameSize];
    char fname[45];
    ...
    while (hwin != INVALID_HANDLE_VALUE) {
        ...
        if (! CxGrabVideoFrame(SMX,frame,sizeof(frame)) ) {
            error("! CxGrabVideoFrame(SMX,frame,size)");
            PostMessage(hwin,WM_CLOSE,0,0);
            continue;
        }
        ...
        CxBayerToRgb( frame, Params.Width, Params.Height, 2, frameOut );
