...
/*
    In this interpolation method each interpolated output pixel is assigned 
    the value of the nearest pixel in the input image. The nearest neighbor 
    can be any one of the upper, lower, left and right pixels. 
*/
void AlgNearestNeighbor(UCHAR *in, int W, int H, RGBFrame *out) {
    register int idxH, idxW;
    UCHAR *in1 = in;
    UCHAR *in2 = in + W;
    RGBFrame *out1 = out;
    RGBFrame *out2 = out + W;
...
/*
    ...
    B10 G11 
*/
    for (idxH = 0; idxH < H; idxH += 2, in1 += W, in2 += W, out1 += W, out2 += W) {
        for (idxW = 0; idxW < W; idxW += 2, out1 += 2, out2 += 2 ) {
            register UCHAR G00,B10,R01,G11;
...
static void ThreadProc(void *foo) {
    static RGBFrame frameOut[1400*1024];
    char fname[45];
