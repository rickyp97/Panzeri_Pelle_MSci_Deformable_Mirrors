...
def setvalue(value, Dac, channel, valueflag):
    board = dac.DAC1
    if Dac == 'DAC2':
        board = dac.DAC2
    if valueflag == "Voltage":
        dac.setVoltage(board, channel, value)
        print("voltage is" , value, 'on', Dac)
        return
    elif valueflag == "Duty":
        dac.setValue(board, channel, value)
        print("duty is", value, 'on', Dac)
    else:
        print ('error')
        return

def setall(channellist, Dac, valueflag):
    for i in range(0,len(channellist),1):
        setvalue(int(channellist[i].get()), Dac, i+1, valueflag)
    return
...
if __name__ == "__main__":    
    ...
    tk.Button(text = 'set', command = lambda: setvalue(int(ch1.get()),boardflag.get() ,1,valueflag.get())).grid(row = 2, column = 5)
    tk.Button(text = 'set', command = lambda: setvalue(int(ch2.get()),boardflag.get() ,2,valueflag.get())).grid(row = 3, column = 5)
    tk.Button(text = 'set', command = lambda: setvalue(int(ch3.get()),boardflag.get() ,3,valueflag.get())).grid(row = 4, column = 5)
    tk.Button(text = 'set', command = lambda: setvalue(int(ch4.get()),boardflag.get() ,4,valueflag.get())).grid(row = 5, column = 5)
    tk.Button(text = 'set', command = lambda: setvalue(int(ch5.get()),boardflag.get() ,5,valueflag.get())).grid(row = 6, column = 5)

    tk.Button(text = 'set all', command = lambda: setall(channellist, boardflag.get(),valueflag.get())).grid(row = 7, column = 4)
